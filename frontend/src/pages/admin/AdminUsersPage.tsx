...
export function AdminUsersPage() {
  ...
  const filteredInvites = invites.filter((inv) => {
    ...
    if (inviteStatusFilter) {
      const isExpired = inv.expires_at ? new Date(inv.expires_at).getTime() <= currentTime : false;
    ...
    <div className="w-full space-y-5 py-1">
      ...
      {activeTab === 'users' && (
        <UsersTab
          ...
          onResetPassword={(user) => { setResetUser(user); setResetError(null); }}
                                ...
                                <div className="text-[11px] text-text-tertiary">
                                  {isExpired
                                    ...
                                    : `剩 ${Math.max(
                                        1,
                                        Math.ceil(
                                          (new Date(inv.expires_at).getTime() - currentTime) / (1000 * 60 * 60 * 24)
                                        )
                                      )} 天`}
                                </div>
                              </div>
